#ifndef _IPC_POOLALLOC_H_
#define _IPC_POOLALLOC_H_
...
T *alloc(P &&...params)
{
    return construct<T>(PoolAlloc::alloc(sizeof(T)), std::forward<P>(params)...);
}

inline void free(void *p, std::size_t size)
{
    PoolAlloc::free(p, size);
}
