#ifndef _IPC_CIRC_ELEMARRAY_H_
#define _IPC_CIRC_ELEMARRAY_H_

#include <atomic>
#include <limits>
#include <utility>
#include <type_traits>
#include <circ/Head.hpp>
...
class Segment : public ipc::detail::Head<ProdCons>
{
public:
    using base_t   = ipc::detail::Head<ProdCons>;
    using cursor_t = decltype(std::declval<ProdCons>().cursor());
    using elem_t   = typename ProdCons::template elem_t<DataSize, AlignSize>;
...
private:
    ...
    sender_checker<ProdCons, RelationTrait<ProdCons>::is_multi_producer> c_s_;
    receiver_checker<ProdCons, RelationTrait<ProdCons>::is_multi_consumer> c_r_;
};

} // namespace detail
} // namespace ipc

#endif // ! _IPC_CIRC_ELEMARRAY_H_